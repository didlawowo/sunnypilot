...
# Réglage du seuil de largeur (voir docs/regles-depassement.md). Le pas de
# 0,25 m est un compromis tactile : 0,1 demanderait soixante-dix appuis pour
# traverser la plage, 0,5 sauterait par-dessus l'écart qui sépare un
# dépassement légitime d'un faux positif (3,3 m contre 3,7 m mesurés).
REGLAGE_PAS_M = 0.25
REGLAGE_MIN_M = 1.0
REGLAGE_MAX_M = 8.0
REGLAGE_HEIGHT = 90
...
class IoniqControlLayout(Widget):
    ...
    def __init__(self) -> None:
        ...
        self._moins_rect = rl.Rectangle(0, 0, 0, 0)
    ...
    def _render(self, rect: rl.Rectangle) -> None:
        ...
        # Réglage du seuil de largeur, juste au-dessus du bouton
        reglage_y = rect.y + rect.height - BUTTON_HEIGHT - REGLAGE_HEIGHT - 40
        self._draw_reglage(x, reglage_y, rect.width)
    ...
    def _seuil_actuel(self) -> float | None:
        """Seuil appliqué par le daemon, ou None s'il ne le publie pas.

        On n'invente pas de valeur de repli : afficher un chiffre qui n'est pas
        celui qui décide serait pire que d'afficher un tiret.
        """
        if self._state is None or self._state_age > STATE_STALE_S:
            return None
        valeur = self._state.get("suggestion_seuil_largeur_m")
        return float(valeur) if isinstance(valeur, (int, float)) else None

    def _draw_reglage(self, x: float, y: float, width: float) -> None:
        """Ligne « largeur mini à gauche » avec ses deux boutons."""
        seuil = self._seuil_actuel()
        libelle = tr("Depassement - largeur mini a gauche")
        rl.draw_text_ex(
            self._font_med, libelle, rl.Vector2(x, y + 20), LABEL_SIZE, 0, COLOR_TEXT_NORMAL
        )

        self._plus_rect = rl.Rectangle(
            x + width - REGLAGE_BTN_W, y, REGLAGE_BTN_W, REGLAGE_HEIGHT
        )
        self._moins_rect = rl.Rectangle(
            x + width - 2 * REGLAGE_BTN_W - 20, y, REGLAGE_BTN_W, REGLAGE_HEIGHT
        )
        for rect, signe in ((self._moins_rect, "−"), (self._plus_rect, "+")):
            actif = seuil is not None
            rl.draw_rectangle_rounded(
                rect, 0.2, 12, COLOR_BUTTON_BG if actif else COLOR_BUTTON_BG_BUSY
            )
            taille = rl.measure_text_ex(self._font_bold, signe, LABEL_SIZE, 0)
            rl.draw_text_ex(
                self._font_bold,
                signe,
                rl.Vector2(
                    rect.x + (rect.width - taille.x) / 2,
                    rect.y + (rect.height - taille.y) / 2,
                ),
                LABEL_SIZE,
                0,
                COLOR_BUTTON_TEXT if actif else COLOR_TEXT_NORMAL,
            )

        texte = f"{seuil:.2f} m" if seuil is not None else "-"
        taille = rl.measure_text_ex(self._font_bold, texte, VALUE_SIZE, 0)
        rl.draw_text_ex(
            self._font_bold,
            texte,
            rl.Vector2(self._moins_rect.x - taille.x - 30, y + 20),
            VALUE_SIZE,
            0,
            COLOR_TEXT_NORMAL,
        )

    def _envoyer_reglage(self, valeur: float) -> None:
        payload = {
            "type": "reglage_set",
            "data": {"cle": "seuil_largeur_m", "valeur": round(valeur, 2)},
            "ts": time.time(),
        }
    ...
    def _handle_mouse_press(self, mouse_pos: MousePos) -> None:
        if self._reglage_clique(mouse_pos):
            return
    ...
    def _reglage_clique(self, mouse_pos: MousePos) -> bool:
        """Traite un appui sur − ou +. Rend True s'il en était un.

        Sans seuil publié on ne touche à rien : incrémenter une valeur qu'on ne
        connaît pas reviendrait à en inventer une.
        """
        seuil = self._seuil_actuel()
        if seuil is None:
            return False
        if rl.check_collision_point_rec(mouse_pos, self._moins_rect):
            self._envoyer_reglage(max(REGLAGE_MIN_M, seuil - REGLAGE_PAS_M))
            return True
        if rl.check_collision_point_rec(mouse_pos, self._plus_rect):
            self._envoyer_reglage(min(REGLAGE_MAX_M, seuil + REGLAGE_PAS_M))
            return True
        return False
